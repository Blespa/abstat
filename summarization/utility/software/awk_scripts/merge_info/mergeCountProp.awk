BEGIN {
	...
	while (getline < (dataForCompDir"/concPropSUBJ.txt"))
	{
	...
	while (getline < (dataForCompDir"/concPropOBJ.txt"))
	{
	...
	while (getline < (dataForCompDir"/countSUBJ.txt"))
	{
		split("",resSubjSpl);
		split($0,resSubjSpl,"##");
	...
	while (getline < (dataForCompDir"/countOBJ.txt"))
	{
		split("",resObjSpl);
		split($0,resObjSpl,"##");
...
END { 
	...
	print "Property##How many times is used##Number of Subject Resources##Number of Object Resources##Number of minimum type class that are subject of the property##Number of minimum type class that are subject of the property" >> destDir"/"destFile;
